...
export async function generateDetailedSummary(
  ...
  const extractionTypes = [
    ...
    },
    {
      key: 'keywords',
      description:
        'Key terms and phrases from the content. You can think of them as hastags that increase searchability of the content for the reader. Example of keyword: OpenAI, Large Language Model, API, Agent, etc.',
    },
    {
      key: 'links',
      description:
  ...
  const extractionPromises = extractionTypes.map(({ key, description }) =>
    extractInformation(title, article, key, description).then((result) => {
      return {
        type: key,
        description,
  ...
  await Promise.all(
    extractedResults.map(async ({ type, content }, index) => {
      extractedData[type] = content;
      await writeFile(
        join(__dirname, `${index + 1}_${type}.md`),
        content,
        'utf8'
      );
    })
  ...
  const draftPromise = draftSummary(
    title,
    article,
  ...
  const critique = await critiqueSummary(
    draft,
    article,
  ...
  await writeFile(join(__dirname, 'summary.md'), finalSummaryContent, 'utf8');
